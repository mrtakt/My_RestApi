...
router.post('/signup', recaptcha.middleware.verify, captchaRegister, async(req, res) => {
    ...
    if (!email || !username || !password || !confirmpassword) {
        ...
        req.flash('error_messages',"Password Harus Mengandung Minimal 1 Angka Dan Huruf Besar.Minimal 8 Karakter Tidak Boleh Ada Emoji");
        res.redirect('/signup');  
...
router.get('/send-verification-email', checkAuth, async (req, res) => {
    ...
    if (req.user.isVerified ) {
        ...
        req.flash('success_messages', 'Link Telah Dikirim,Expire Dalam 30 menit')
        res.redirect('/dashboard');
        }
...
if (userData) {
if (Cooldown) {
    req.flash('error_messages','Jangan Spam Tunggu 30 Menit Untuk Melanjutkan');
    res.redirect('/forgot-password')
            ...
            req.flash('success_messages','Cek Email Mu Untuk Informasi Lebih Lanjut, Tunggu 30 Menit Setelah Submit');
            res.redirect('/forgot-password');    
            }
